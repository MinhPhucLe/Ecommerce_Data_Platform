...
for i in range(2, 20):
    ...
    target_purchase = now - timedelta(days=0)
    target_time1 = now - timedelta(days=1)
    target_time2 = now - timedelta(days=2)
    ...
    result_date1 = START_2019 + timedelta(days=days_passed1) + timedelta(days=i)
    day_query1 = result_date1.strftime("%Y%m%d")

    days_passed2 = (target_date2 - START_2025).days
    result_date2 = START_2019 + timedelta(days=days_passed2) + timedelta(days=i)
    day_query2 = result_date2.strftime("%Y%m%d")

    days_passed_purchase = (target_date_purchase - START_2025).days
    result_date_purchase = START_2019 + timedelta(days=days_passed_purchase) + timedelta(days=i)
